...
USE 데이터베이스명;
-- 컬럼 변경하기
ALTER TABLE 테이블명 CHANGE COLUMN 컬럼명 새 컬럼명 데이터타입;
...
DESCRIBE  테이블 명 ;
-- 데이터 추가
INSERT INTO 테이블 명 (칼럼1, 칼럼2, 칼럼3) VALUES(값1, 값2, 값3)
...
### 6.8. authors 테이블의 id가 1 인 데이터를 posts 테이블에 추가
...
-- FK 가 있는 경우
describe posts;
insert into posts (id, title, contents, authors_id) values (1, "제목1", "내용1", 1);
select * from posts;
```
- FK 가 없는 경우에 데이터오류가 발생하며 데이터 추가 안됨.
...
- 여러 칼럼을 정렬함. (앞쪽 처리 후, 나머지 순으로 처리 됨)
...
- 가장많이사용
- 주문시간, 글 작성 시간 등
- `칼럼명 datetime default current_timestamp()` : 현재시간을 기본으로
